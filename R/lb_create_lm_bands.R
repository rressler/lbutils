...
#'
#' This function takes formula input for a linear model and the type of
#' confidence band and alpha level and creates a data frame of values that can
#' be plotted to show the confidence bands.
#'
#' @param formula a formula for a linear model with variables
#' from the data data frame
#' @param data a data frame containing at least three observations of two
#' variables for a linear regression
#' @param band_type a character for the type of band desired. Includes:
#' "whbands".
#' @param level a number between 0 and 1 for the confidence level
...
#' lb_create_lm_bandsf(mpg ~ disp, data = mtcars)
 lb_create_lm_bandsf <- function(formula, data,
